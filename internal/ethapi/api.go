...
func (s *PrivateAccountAPI) signTransaction(ctx context.Context, args *SendTxArgs, passwd string) (*types.Transaction, error) {
    ...
    if err := args.setDefaults(ctx, s.b); err != nil {
		fmt.Printf(" args.setDefaults : err = %v\n", err);	// yhheo
        return nil, err
    }
...
func (s *PrivateAccountAPI) SignTransaction(ctx context.Context, args SendTxArgs, passwd string) (*SignTransactionResult, error) {
    ...
    if err != nil {
		fmt.Printf(" rlp.EncodeToBytes : err = %v\n", err);	// yhheo
        return nil, err
    }
...
func (s *PrivateAccountAPI) Sign(ctx context.Context, data hexutil.Bytes, addr common.Address, passwd string) (hexutil.Bytes, error) {
    fmt.Printf("\nfunc (s *PrivateAccountAPI) Sign\n ctx = %v\n ndata = %x\n addr = %x\n", ctx, data, addr)    // yhheo
	// Look up the wallet containing the requested signer
	account := accounts.Account{Address: addr}

	wallet, err := s.b.AccountManager().Find(account)
	if err != nil {
		fmt.Printf(" s.b.AccountManager : err = %v\n", err);	// yhheo
		return nil, err
	}
	// Assemble sign the data with the wallet
	signature, err := wallet.SignHashWithPassphrase(account, passwd, signHash(data))
	if err != nil {
		fmt.Printf(" wallet.SignHashWithPassphrase : err = %v\n", err);	// yhheo
		return nil, err
	}
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
    fmt.Printf("\nfunc (s *PrivateAccountAPI) EcRecover\n ctx = %v\n data = %x\n sig = %x\n", ctx, data, sig)    // yhheo
	if len(sig) != 65 {
...
func (s *PublicBlockChainAPI) GetBalance(ctx context.Context, address common.Address, blockNr rpc.BlockNumber) (*big.Int, error) {
	...
	if state == nil || err != nil {
		fmt.Printf(" s.b.StateAndHeaderByNumber : err = %v\n", err);	// yhheo
		return nil, err
	}
...
func (s *PublicBlockChainAPI) GetBlockByHash(ctx context.Context, blockHash common.Hash, fullTx bool) (map[string]interface{}, error) {
    fmt.Printf("\nfunc (s *PublicBlockChainAPI) GetBlockByHash\n ctx = %v\n blockHash = %x\n fullTx = %t\n", ctx, blockHash, fullTx)    // yhheo
...
func (s *PublicBlockChainAPI) GetCode(ctx context.Context, address common.Address, blockNr rpc.BlockNumber) (string, error) {
	...
	if len(res) == 0 || err != nil { // backwards compatibility
		fmt.Printf(" state.GetCode : err = %v\n", err);	// yhheo
		return "0x", err
	}
...
func (s *PublicBlockChainAPI) EstimateGas(ctx context.Context, args CallArgs) (*hexutil.Big, error) {
	...
	if (*big.Int)(&args.Gas).Sign() != 0 {
		...
		if err != nil {
			fmt.Printf(" s.b.BlockByNumber : err = %v\n", err);	// yhheo
			return nil, err
		}
...
func (s *PublicTransactionPoolAPI) GetRawTransactionByHash(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
    fmt.Printf("\nfunc (s *PublicTransactionPoolAPI) GetRawTransactionByHash\n ctx = %v\n hash = %x\n", ctx, hash)    // yhheo
	var tx *types.Transaction
	var err error
...
func (s *PublicTransactionPoolAPI) sign(addr common.Address, tx *types.Transaction) (*types.Transaction, error) {
    fmt.Printf("\nfunc (s *PublicTransactionPoolAPI) sign\n addr = %x\n tx = %v\n", addr, tx)    // yhheo
	// Look up the wallet containing the requested signer
	account := accounts.Account{Address: addr}
...
func (args *SendTxArgs) setDefaults(ctx context.Context, b Backend) error {
	...
	if args.GasPrice == nil {
		...
		if err != nil {
			fmt.Printf(" b.SuggestPrice : err = %v\n", err);	// yhheo
			return err
		}
	...
	if args.Nonce == nil {
		...
		if err != nil {
			fmt.Printf(" b.GetPoolNonce : err = %v\n", err);	// yhheo
			return err
		}
...
func (s *PublicTransactionPoolAPI) SendTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	if err != nil {
		fmt.Printf(" wallet.SignTx : err = %v\n", err);	// yhheo
		return common.Hash{}, err
	}
	types.TxChecking(types.MakeSigner(s.b.ChainConfig(), s.b.CurrentBlock().Number()), signed)		// yhheo
