...
func newPoDCProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, maxPeers int, mux *event.TypeMux, txpool txPool, engine consensus.PoDC, blockchain *core.BlockChain, chaindb ethdb.Database) (*istanbulProtocolManager, error) {
