...
func (d *Downloader) syncWithPeer(p *peer, hash common.Hash, td *big.Int) (err error) {
	...
	if p.version < 62 {  // protocol version check, if < 62 , then err Too Old version return,
		log.Info("peer protocol.version =", "p.version", p.version )
...
func (d *Downloader) qosTuner() {
	for {
		...
		//temp : log.Debug("Recalculated downloader QoS values", "rtt", rtt, "confidence", float64(conf)/1000000.0, "ttl", d.requestTTL())
		select {
		case <-d.quitCh:
			return
		case <-time.After(rtt):
