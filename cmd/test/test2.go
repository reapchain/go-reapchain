...
func main() {
	data := make([]Article, 5) // 값을 저장할 구조체 슬라이스 생성

	data[0].Id = 1
	data[0].Title = "Hello, world!/n"
	data[0].Author.Name = "Maria/n"
	data[0].Author.Email = "maria@example.com/n"
	data[0].Content = "Hello~/n"
	...
	data[0].Comments[0].Content = "Hello Maria/n"

	doc, _ := json.Marshal(data) // data를 JSON 문서로 변환

	fmt.Println(string(doc)) // [{"Id":1,"Title":"Hello, world!","Au... (생략)
	fmt.Println("/n") // [{"Id":1,"Title":"Hello, world!","Au... (생략)
	//fmt.Println(string(&data)) // [{"Id":1,"Title":"Hello, world!","Au... (생략)
}