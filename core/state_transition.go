...
func (st *StateTransition) TransitionDb() (ret []byte, requiredGas, usedGas *big.Int, failed bool, err error) {
	...
	//disableGasMetering  := st.evm.VmConfig().DisableGasMetering
	estimateGasMetering := st.evm.VmConfig().EstimateGasMetering
	...
	if contractCreation {
		if isGovernance || estimateGasMetering {
			ret, _, st.gas, vmerr = evm.Create(sender, st.data, st.gas, st.value)
		} else {
			log.Debug("no contract creation permissions", "sender", sender)
			return nil, nil, nil, false, vm.ErrDeployPermissions
		}
	} else {
...
func (st *StateTransition) refundGas() {
    ...
    if !isGovernance && !isCoinTransfer {
		remaining = new(big.Int).Mul(new(big.Int).SetUint64(st.gas), big.NewInt(params.Gasfee))
		...
		refundFee := new(big.Int).Mul(refund, big.NewInt(params.Gasfee))
